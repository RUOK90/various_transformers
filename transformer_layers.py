...
def get_normalized_attention(attn_norm, scores):
    if attn_norm == 'softmax':
        return F.softmax(scores, dim=-1)
    elif attn_norm == 'tanh':
        return F.tanh(scores)
    elif attn_norm == 'sigmoid':
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        if self.attn_type == 'vanilla':
            ...
            scores = get_sparse_scores(self.sparsity_mode, self.sparsity_top_k, scores)
            if mask is not None:
                scores = scores.masked_fill(mask == 0, -1e9)
            p_attn = F.softmax(scores, dim=-1)
            ...
            scores = self.w(F.relu(query)).view(nbatches, self.h, seq_len, self.max_len)[:, :, :, :seq_len]
            scores = get_sparse_scores(self.sparsity_mode, self.sparsity_top_k, scores)
            if mask is not None:
                scores = scores.masked_fill(mask == 0, -1e9)
            ...
            value = self.linears[0](value).view(nbatches, self.h, seq_len, self.d_k)
            scores = self.r[:, :seq_len, :seq_len]
            scores = get_sparse_scores(self.sparsity_mode, self.sparsity_top_k, scores)
            if mask is not None:
                scores = scores.masked_fill(mask == 0, -1e9)
            p_attn = get_normalized_attention(self.attn_norm, scores)
            ...
            random_scores = self.r[:, :seq_len, :seq_len]
            scores = get_sparse_scores(self.sparsity_mode, self.sparsity_top_k, dense_scores + random_scores)
            if mask is not None:
                scores = scores.masked_fill(mask == 0, -1e9)
            p_attn = get_normalized_attention(self.attn_norm, scores)
